...
from MuchPool.Notears import NotearsWithMask
...
class DegreePickBlock(object):
    ...
    def forward(self, X, adj, mask, assign_matrix, H_coarse):
        ...
        '''
        k_max = int(math.ceil(self.filt_percent * adj.shape[-1]))#每批次中筛选节点的最大数量。
        k_list = [int(math.ceil(self.filt_percent * x)) for x in mask.sum(dim=1).tolist()]#为每个样本计算筛选节点的数量，依赖于掩码 mask
        ...
        new_mask = X.new_zeros(X.shape[0], k_max)# 用于标记筛选后保留的节点
        S_reserve = X.new_zeros(X.shape[0], k_max, adj.shape[-1])# 用于保存筛选后的节点的邻接矩阵
        inter_channel_adj = X.new_zeros(X.shape[0], k_max, cluster_num)# 跨通道邻接矩阵
...
class CasualPickBlock(object):
    def __init__(self, config):
            self.filt_percent = config.percent
            self.assign_ratio = config.diffPool_assign_ratio
            self.max_node_num = config.diffPool_max_num_nodes
            self.inter_channel_gcn = InterChannelGCN(config.hidden_dim, config.hidden_dim)
            self.notears_model = NotearsWithMask()  # 使用因果图学习模块
    ...
    def forward(self, X, adj, mask, assign_matrix, H_coarse):
            ...
            input:
                ...
                H_coarse: embedding matrix of coarse graph, [batch, cluster_num, hidden_dim], dtype=float
            outputs:
                top_index: indices of selected nodes after filtering, [batch, top_k]
                H: batch of node hidden features, [batch, node_num, hidden_dim]
                k_list: list of top_k values for each batch
            '''
            top_indices = []
            k_list = []
            ...
            for batch_idx in range(X.shape[0]):
                # 提取单个样本的特征和邻接矩阵
                node_features = X[batch_idx].cpu().detach().numpy()
                adj_matrix = adj[batch_idx].cpu().detach().numpy()
                adj_matrix = (np.abs(adj_matrix) > 0).astype(int)
                # 使用 Notears 学习因果结构
                causal_matrix = self.notears_model.learn(node_features, adj_matrix)

                # 去掉叶子节点
                in_degrees = np.sum(causal_matrix, axis=0)
                leaf_nodes = np.where(in_degrees == 0)[0]
                non_leaf_nodes = np.setdiff1d(np.arange(causal_matrix.shape[0]), leaf_nodes)

                # 保存选择的节点索引
                top_indices.append(torch.tensor(non_leaf_nodes, dtype=torch.long))
                k_list.append(len(non_leaf_nodes))  # 每个样本保留节点数量

            # 堆叠批次结果
            top_index = torch.stack(top_indices, dim=0).to(X.device)

            # 更新嵌入
            new_adj_matrices = torch.stack(
                [torch.tensor(self.notears_model.causal_matrix, dtype=torch.float32)] * X.shape[0]).to(X.device)
            H = torch.matmul(new_adj_matrices, X)
            H = self.inter_channel_gcn(H, H_coarse, assign_matrix)
